...
There could be major outliers that skew the data if you use the mean.
...
Binwidth 1000 was too small to see, and 5000 seemed like a good width to see what was going on.
...
The max and the min are important to determine the range of the data. The mean is helpful because it shows you where most of the data lies. The data is skewed to the right, so there are outliers on the right.
...
ggplot(college_recent_grads, aes(x = median)) +  # Use median_income for the x-axis
  geom_histogram(binwidth = 5000) + 
...
Interdisciplinary is the least popular major category
...
There are more non-setm majors but the few women in stem majors make more money
...
What are the proportion of men vs women in stem/non-stem majors?

```{r}
ggplot(college_recent_grads, aes(x = women, y = men, color = major_type)) + 
  geom_point(alpha = 0.7) + 
  labs(
    title = "Proprtion of Men vs. Proportion of Women in Major",
    x = "Proportion of Women in Major",
    y = "Proportion of Men in Major",
    color = "Major Category"
  ) 
```
